...
program.option("-i, --input <input>", "input file path");
program.parse(process.argv);

const CWD = process.cwd();
const options = program.opts();
const filePath = path.join(CWD, options.input);
...
traverse(ast, {
    ...
    ExportDefaultDeclaration(path) {
        ...
        if (node.type !== "ExportDefaultDeclaration" || declaration.type !== "ObjectExpression") {
            return;
        }
        ...
        const importSpecifierOpt = ["Component"];
        // mixns
        const mixinsOpt = [];
        let componentsOpt = [];
        let className = "";
        ...
        properties.forEach((item) => {
                    ...
                    if (item.value.type === "ObjectExpression") {
                        ...
                        props.forEach((prop) => {
                            ...
                            if (prop.value.type === "ObjectExpression") {
                                ...
                                const typeItem = propItem.find((item) => item.key.name === "type");
                        ...
                        if (
                            ...
                            eventItem.forEach((item, index) => {
                                ...
                                if (item.type === "StringLiteral") {
                                    const Node = t.classMethod(
                                        "method",
                                        t.identifier(`on${watchFnName(item.value)}${index}Change`),
                                        [t.identifier("newVal"), t.identifier("oldVal")],
                                        t.blockStatement([
                                            t.expressionStatement(
                                                t.callExpression(
                                    ...
                                    Node.decorators = [
                                        t.decorator(t.identifier(`Wacth('${watchItem.key.name}')`)),
                                    ];
                                    classMethods.push(Node);
                                }
                                // 数组里面是函数
            ...
            if (item.type === "ObjectMethod") {
                ...
                if (item.key.name === "data") {
                    const data = item.body.body.filter((item) => item.type === "ReturnStatement")[0]
                        .argument.properties;
                    data.forEach((item) => {
        ...
        const hasMixinsStr = mixinsOpt.length ? `extends Mixins(${mixinsOpt.join(",")})` : "Vue";
            ...
            t.classDeclaration(
                ...
                t.classBody([
                    t.classProperty(
                        ...
                        null,
                        t.typeAnnotation(t.genericTypeAnnotation(t.identifier("any | undefined"))),
                        [],
                        false,
                        false
                    ),
...
function getOutFilePath(fileName:string) {
